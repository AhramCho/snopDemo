 ...
 *  - 실제 계산(수요계획/공급계획/MRP 등)은 전혀 하지 않고,
...
// 파이프라인 상단에 표시할 6개 스텝 정의 (라벨/부제만 담당하는 화면용 메타데이터)
const stationDefs = [
 ...
 * 이 객체는 PlanningEngine.computeMRPCascade() 등에 그대로 전달됩니다.
...
function runAll() {
  ...
  const mrp = PlanningEngine.computeMRPCascade(M, demandPlan, WEEKS);
  const metrics = PlanningEngine.computeReportMetrics(M, demandPlan, supplyPlan, mrp.planFG, mrp.planSFG);

  // ---- 결과를 화면에 렌더링 ----
  renderRouting(M, mrp.LT_FG, mrp.LT_SFG, mrp.LT_RM, mrp.LT_LOGI);
  renderDemandPlan(forecast, adj, demandPlan);
  renderSupplyPlan(demandPlan, supplyPlan, supplyGap, invAfterSupply, M);
  renderProductionPlan(M, mrp.planFG, mrp.utilFG, mrp.LT_FG, mrp.shipLateFlag);
  renderMRP(M, mrp.planSFG, mrp.planRM, mrp.utilSFG, mrp.LT_SFG, mrp.LT_RM);
  renderReport(M, demandPlan, supplyPlan, mrp.planFG, mrp.planSFG, mrp.utilFG, mrp.utilSFG, metrics);
}
...
function renderReport(M, demandPlan, supplyPlan, planFG, planSFG, utilFG, utilSFG, metrics) {
